...
static char* test_display_format() {
    ...
    time_init(&t, 9, 5, 30);
    char* result = time_display(&t);
    mu_assert("Format incorrect pour 09:05:30", strcmp(result, "09:05:30") == 0);
    
    // Test 2: Minuit
    time_init(&t, 0, 0, 0);
    result = time_display(&t);
    mu_assert("Format incorrect pour 00:00:00", strcmp(result, "00:00:00") == 0);
    
    // Test 3: Midi
    time_init(&t, 12, 0, 0);
    result = time_display(&t);
    mu_assert("Format incorrect pour 12:00:00", strcmp(result, "12:00:00") == 0);
    
    // Test 4: Cas limite (23:59:59)
    time_init(&t, 23, 59, 59);
    result = time_display(&t);
    mu_assert("Format incorrect pour 23:59:59", strcmp(result, "23:59:59") == 0);
    
    // Test 5: Pointeur NULL
    result = time_display(NULL);
    mu_assert("Gestion incorrecte de NULL", strcmp(result, "INVALID") == 0);
